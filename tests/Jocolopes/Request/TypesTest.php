...
class TypesTest extends \UnitTestCase {
    ...
    public function testOperatingSystems()
    {
        ...
        {
            $types->setUserAgent($ua);
            $grabbed = $types->getTypes();
            
            if(in_array($platform, ['iOS', 'BlackBerryOS', 'AndroidOS'])){
                $this->assertEquals('mobile', $grabbed[1]);
            }else{
                $this->assertEquals('desktop', $grabbed[1]);
            }
            $this->assertEquals('universal', $grabbed[0]);
            $this->assertEquals(strtolower(str_replace(' ', '', $platform)), $grabbed[2]);
        }
    }
